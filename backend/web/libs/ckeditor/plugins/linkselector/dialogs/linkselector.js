...
CKEDITOR.dialog.add( 'linkselectorDialog', function ( editor ) {
    ajaxUrls = editor.config.linkselectorAjaxUrls;
    modelTypes = editor.config.linkselectorModelTypes;
    defaultModelType = modelTypes.find(function (item) {return item.indexOf("default") > -1});
    return {
                ...
                elements: [
                        ...
                        children: [
                            {
                                ...
                                label: 'Model Type',
                                items: modelTypes,
                                default: defaultModelType ? defaultModelType[1] : "",
                            ...
                            },
                            {
                                ...
                                commit: function( element ) {
                                    ...
                                    element.setAttribute( "href", modelUrl );
                                }
                            }
                        ]
                    },
                    {
                        ...
                        onChange: function () {
                            ...
                            if (keyword) {
                                ckeContentElement( 'tab-basic', 'text' ).setValue( keyword );
                                ckeContentElement( 'tab-basic', 'title' ).setValue( keyword );
                            }
                        }
                    },
                    {
        ...
        onShow: function() {
            ...
            if (this.insertMode) {
                ...
                modelUrl = element.getAttribute('href') || "";
        ...
        onOk: function() {
            var dialog = this;
            var elm = dialog.element;

            dialog.commitContent( elm );
...
function select2(selector) {
        ...
        return (
            '<div class="select2-option-row">' +
                ...
                '<div class="select2-option-row--right">' +
                    '<div class="__name">' + repo.text + '</div>' +
                    '<div class="__url">' + repo.id + '</div>' +
                '</div>' +
            '</div>'
        );
    }
    ...
    function formatRepoSelection (repo) {
        ...
        return modelName + ' &rightarrow; ' + modelUrl;
    }
    ...
    $(selector).on("select2:select", function (event) {
        ...
        //         }
                ...
                if (keywords.length > 0) {
                    keywords.forEach(function (keyword) {
                        keyword_sel.add(keyword, keyword);
                    });
                    keyword_sel.enable();
